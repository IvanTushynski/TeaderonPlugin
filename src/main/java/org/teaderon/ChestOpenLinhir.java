    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (check >= 0 && check <= 20) {
                ...
                this.executeCommand4(player, true, "mi give MATERIAL LEATHER2 %player% 20");
                player.playSound(player.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 1.0F, 1.0F);
                player.sendTitle(ChatColor.GOLD + "Лисья Шкура 20 штук!", ChatColor.RED + "Поздравляем", 2, 30, 2);
            } else if (check >= 241 && check <= 260) {
                this.executeCommand4(player, true, "mi give MATERIAL LEATHER2 %player% 30");
                player.playSound(player.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 1.0F, 1.0F);
                player.sendTitle(ChatColor.GOLD + "Лисья Шкура 30 штук!", ChatColor.RED + "Поздравляем", 2, 30, 2);
            }
        }
